...
import 'package:stacionar_app/features/core/navigation_tabs/applications_list/fortune_wheel/painters/wheel_outline_painter.dart';
...
import 'package:stacionar_app/features/core/navigation_tabs/applications_list/fortune_wheel/widgets/wheel_result_indicator.dart';
import 'package:stacionar_app/features/core/navigation_tabs/applications_list/fortune_wheel/widgets/wheel_slice.dart';
...
class FortuneWheel<T> extends StatefulWidget {
  ...
  final List<FortuneWheelModel<T>> children;
...
class _FortuneWheelState extends State<FortuneWheel> with SingleTickerProviderStateMixin {
  ...
  void _initiateAnimation() {
    ...
    _animationController.addListener(() {
      widget.controller.setValue(widget.children[((widget.children.length) * (_animationController.value % 1)).floor()]);
  ...
  void _startAnimation() {
    ...
    int milliseconds =
        Random().nextInt(widget.rotationTimeLowerBound) + (widget.rotationTimeUpperBound - widget.rotationTimeLowerBound);
  ...
  WheelResultIndicator _getIndicator() =>
      WheelResultIndicator(wheelSize: size, animationController: _animationController, childCount: widget.children.length);
      ...
      children: [
        for (int index = 0; index < widget.children.length; index++)
          Transform.rotate(
            ...
            child: WheelSlice(index: index, size: size, fortuneWheelChildren: widget.children),
          ),
      ],
    );
  }
